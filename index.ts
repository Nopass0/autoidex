...
interface Transaction {
  ...
  wallet:
    ...
    | string; // Может быть объектом или строкой для обратной совместимости
  amount: any;
  total: any;
  status: number;
  approved_at?: string;
...
async function processPendingIdexSyncOrders(): Promise<number> {
  try {
    ...
    console.info(
      `Найдено ${pendingOrders.length} отложенных IDEX ордеров на синхронизацию`
    );
      ...
      try {
        ...
        for (const cabinet of cabinetsToSync) {
          try {
            ...
            const pagesToFetch = determinePages(
              order.pages,
              cabinetsToSync.indexOf(cabinet)
            );
            console.info(
              `Синхронизация кабинета ID ${cabinet.id} (логин: ${cabinet.login}) с ${pagesToFetch} страницами`
            );
            ...
            console.info(
              `Завершена синхронизация кабинета ID ${cabinet.id}: сохранено ${savedTransactions.length} новых транзакций`
            );
          } catch (cabinetError) {
            console.error(
              `Ошибка синхронизации кабинета ID ${cabinet.id}: ${cabinetError.message}`
            );
            processedResults.push({
              cabinetId: cabinet.id,
        ...
        console.error(
          `Ошибка обработки ордера синхронизации ID ${order.id}: ${orderError.message}`
        );
    ...
    console.error(
      `Необработанная ошибка в processPendingIdexSyncOrders: ${error.message}`
    );
    return 0;
  }
...
async function determineCabinetsToSync(order: any) {
  if (order.cabinetId) {
    ...
    const cabinet = await withRetry(() =>
      prisma.idexCabinet.findUnique({
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (retryCount >= MAX_RETRIES) {
          throw new Error(
            "Превышено максимальное количество попыток. Последний статус: 429 Too Many Requests"
          );
        }

        const retryAfter = parseInt(
          response.headers["retry-after"] || String(delay)
        );
        console.warn(
          `Ограничение скорости (429). Ожидание ${retryAfter}мс перед повторной попыткой. Попытка ${
            retryCount + 1
          }/${MAX_RETRIES}`
        );
        ...
        throw new Error(
          `Авторизация не удалась со статусом: ${response.status}`
        );
      }
    } catch (error: any) {
      if (error.response?.status === 429) {
        ...
        const retryAfter = parseInt(
          error.response.headers["retry-after"] || String(delay)
        );
        console.warn(
          `Ограничение скорости (429). Ожидание ${retryAfter}мс перед повторной попыткой. Попытка ${
            retryCount + 1
...
async function fetchTransactionsPage(
  ...
  // Упрощенный URL без фильтров, как в новом API
  const transactionsUrl = `${BASE_URL}/api/v1/payments/payouts?page=${page}`;
  console.info(`Запрос транзакций: ${transactionsUrl}`);
  ...
  while (true) {
    try {
      ...
      if (response.status === 200) {
        ...
        // Проверяем новый формат ответа (success: true, response.payouts.data)
        if (
          json.success === true &&
          json.response?.payouts?.data &&
          Array.isArray(json.response.payouts.data)
        ) {
          data = json.response.payouts.data;
          console.info(
            `Используется новый формат API ответа. Получено транзакций: ${data.length}`
          );

          // Логируем пример первой транзакции для отладки
          if (data.length > 0) {
            const firstTransaction = data[0];
            console.info(`Пример транзакции (ID: ${firstTransaction.id}):`);
            console.info(
              `  - wallet: ${JSON.stringify(firstTransaction.wallet)}`
            );
            console.info(`  - status: ${firstTransaction.status}`);
            console.info(
              `  - amount: ${JSON.stringify(firstTransaction.amount).substring(
                0,
                100
              )}`
            );
        ...
        // Проверяем старый формат ответа (data массив напрямую)
        else if (Array.isArray(json.data)) {
          data = json.data;
          console.info(
            `Используется старый формат API ответа. Получено транзакций: ${data.length}`
          );
        }
...
async function fetchTransactions(
  cookies: Cookie[],
  pages: number = DEFAULT_PAGES_TO_FETCH,
  db: any
): Promise<Transaction[]> {
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    try {
      ...
      console.info(
        `Найдено ${transactions.length} транзакций на странице ${page}`
      );

      // Проверяем, есть ли транзакции уже в базе данных
      if (transactions.length > 0) {
        const externalIds = transactions.map((t) => t.id);
        const existingTransactions = await withRetry(() =>
          db.idexTransaction.findMany({
            where: {
        ...
        console.info(
          `Найдено ${newTransactions.length} новых транзакций на странице ${page}`
        );
        ...
        if (newTransactions.length === 0) {
          console.info(
            `Все транзакции на странице ${page} уже существуют в базе данных. Прекращаем получение.`
          );
          break;
        }
      }
...
async function saveTransactions(
  ...
  console.info(
    `Найдено ${existingTransactions.length} существующих транзакций для кабинета ${cabinetId}`
  );
  ...
  const newTransactions = transactions.filter(
    (t) => !existingPairs.has(`${t.id}_${cabinetId}`)
  );

  if (newTransactions.length === 0) {
    console.info(
      `Нет новых транзакций для сохранения для кабинета ${cabinetId}`
    );
    return [];
  }
  ...
  const savedTransactions = await Promise.all(
    newTransactions.map(async (transaction) => {
      ...
      // Извлекаем значение wallet в зависимости от формата
      let walletValue: string;
      if (typeof wallet === "string") {
        // Старый формат - wallet уже строка
        walletValue = wallet;
      } else if (typeof wallet === "object" && wallet !== null) {
        // Новый формат - wallet это объект с phone или card
        walletValue = wallet.phone || wallet.card || JSON.stringify(wallet);
        // Логируем только для первой транзакции
        if (newTransactions.indexOf(transaction) === 0) {
          console.info(
            `Обработка wallet объекта: ${JSON.stringify(
              wallet
            )} -> ${walletValue}`
          );
        }
        ...
        // Резервный вариант
        walletValue = JSON.stringify(wallet);
        console.warn(
          `Неожиданный формат wallet: ${typeof wallet}, значение: ${wallet}`
        );
      }

      return withRetry(() =>
        db.idexTransaction.create({
          data: {
            ...
            approvedAt: approved_at
              ? new Date(
                  new Date(approved_at).getTime() + 3 * 60 * 60 * 1000
                ).toISOString()
              : null,
            expiredAt: expired_at
              ? new Date(
                  new Date(expired_at).getTime() + 3 * 60 * 60 * 1000
                ).toISOString()
              : null,
  ...
  console.info(
    ...
    } новых транзакций для кабинета ${cabinetId} (всего: ${
      existingTransactions.length + savedTransactions.length
    })`
  );
...
async function withRetry<T>(
  ...
  try {
    ...
    if (
      ...
      console.info(
        `Проблема с подключением к базе данных. Повторная попытка через ${delay}мс. Осталось попыток: ${retries}`
      );
...
async function createAutoSyncOrder(): Promise<number | null> {
  try {
    ...
    console.info(
      `Создано автоматическое задание на синхронизацию всех кабинетов (ID: ${newOrder.id})`
    );
    return newOrder.id;
  } catch (error) {
    console.error(
      `Ошибка при создании автоматического задания: ${error.message}`
    );
    return null;
  }
}
...
async function watchForSyncOrders(checkInterval: number = 10000) {
  console.info(
    `IDEX парсер запущен в режиме ожидания, проверка каждые ${
      checkInterval / 1000
    } секунд`
  );
  ...
  while (isRunning) {
    try {
      ...
      if (pendingOrders.length > 0) {
        console.info(
          `Найдено ${pendingOrders.length} новых заданий на синхронизацию IDEX`
        );
        ...
        if (now.getMinutes() % 10 === 0 && now.getSeconds() < 10) {
          console.info(
            `[${now.toISOString()}] Ожидание новых заданий на синхронизацию IDEX...`
          );

          // Также выводим информацию о следующей автоматической синхронизации
          const nextAutoSyncIn = Math.ceil(
            (autoSyncInterval - (currentTime - lastAutoSyncTime)) / 1000 / 60
          );
          console.info(
            `Следующая автоматическая синхронизация через ${nextAutoSyncIn} минут`
          );
        }
      }
    } catch (error) {
      console.error(
        `Ошибка при проверке заданий на синхронизацию: ${error.message}`
      );
    }
...
watchForSyncOrders().catch((error) => {
